...
<template>
  <div class="flex flex-col items-center text-center py-10 px-4">
    <div class="w-full flex items-center justify-between sm:block">
      ...
      <button class="sm:hidden flex flex-col gap-1" @click="isOpen = true">
  ...
  <!-- Navbar Mobile -->
  <div v-if="isOpen" @click="isOpen = false" class="fixed inset-0 bg-black/40 sm:hidden"></div>

  <div
    class="fixed top-0 right-0 h-full w-64 bg-white shadow-lg p-6 flex flex-col gap-4 transform transition-transform duration-300 sm:hidden"
